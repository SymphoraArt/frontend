...
export default function SetupZkpPage() {
  ...
  if (!ENABLE_RECOVERY_PROTOTYPE) {
    return (
      <div style={{ minHeight: "100vh", background: CREAM, fontFamily: FONT, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ width: "100%", maxWidth: 520, background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 12, padding: 28 }}>
          <p style={{ fontSize: 11, fontFamily: "monospace", letterSpacing: "2px", color: "#a09788", margin: "0 0 12px", textTransform: "uppercase" }}>Recovery evidence disabled</p>
          <h1 style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: 30, fontWeight: 900, color: "#111", margin: "0 0 12px" }}>This flow is not live yet.</h1>
          <p style={{ fontSize: 13, color: "#a09788", margin: "0 0 20px", lineHeight: 1.65 }}>
            The current evidence-hash flow uses a demo hash and no server persistence. It is disabled until a real WebCrypto/KDF plus backend storage path exists.
          </p>
          <a href="/settings?tab=recovery" style={{ display: "block", textAlign: "center", padding: "13px", background: "#111", color: "#fff", borderRadius: 8, textDecoration: "none", fontSize: 14, fontWeight: 600 }}>
            Back to Recovery & 2FA
          </a>
        </div>
      </div>
    );
