    ...
    resources: [
      {
        ...
        method: "POST",
        description:
          "Generate one image. Body: { prompt } for plain text, or { promptId, variableValues } to use an artist's prompt (the artist's share is paid inside your own transaction). Optional: modelFamily (nano-banana-pro | gpt-image-2), resolution (1K|2K|4K), aspectRatio, quality. A request without X-PAYMENT answers 402 with exact requirements: extra.feePayer carries our gas payer (you only need USDC) and extra.legs lists every required TransferChecked — one per recipient, exact amounts. Send the partially signed transaction back in X-PAYMENT; we co-sign, settle and answer with the image.",
        status: "live — plain prompts, free/showcase prompts and priced artist prompts (multi-leg source split)",
      },
    ],
  });
}
