...
/**
 * Enki icon set — ported verbatim from the design bundle
 * (enki-v3-left-side-menu/project/enki/icons.js). Lucide-derived paths rendered
 * as inner SVG so the left menu + node creator match the mockup pixel-for-pixel.
 */
export const ENKI_ICONS: Record<string, string> = {
  ...
  more: '<circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/><circle cx="12" cy="12" r="9"/>',
  ...
  imageDrop: '<rect width="18" height="18" x="3" y="3" rx="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/>',
};
...
export function Icon({ name, size = 22, stroke = 2, className, style, fill = "none" }: IconProps) {
  const path = ENKI_ICONS[name] || "";
  return (
    <svg
      width={size}
      height={size}
