...
import { useTurnkeyPasskey } from "@/hooks/useTurnkeyPasskey";
...
export function WalletPickerModal({ open, onClose }: WalletPickerModalProps) {
  ...
  // Passkey / Turnkey state — the embedded wallet is invisible; the user only
  // sees Face ID / fingerprint.
  const { set: setTurnkeyAuth } = useTurnkeyEmailAuth();
  const { step, error: turnkeyError, walletAddress: turnkeyWalletAddress, subOrganizationId, sessionToken, isReturning, signUp, signIn, reset: resetTurnkey } = useTurnkeyPasskey();
  ...
  useEffect(() => {
    if (step === "done" && turnkeyWalletAddress && subOrganizationId) {
      ...
      toast({
        title: isReturning ? "Welcome back" : "Account ready",
        description: isReturning
          ? "Signed in with your passkey."
          : "Your account is secured by your passkey.",
      });
      handleClose();
    }
      ...
      <DialogContent className="sm:max-w-xs p-4">
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">
            Sign in with a passkey or connect a Solana or EVM wallet.
          </DialogDescription>
        </DialogHeader>

        <div className="mt-2 space-y-1">
          {/* Passkey — the default: no email, no wallet vocabulary. The
              browser's Face ID / fingerprint sheet is the whole flow. */}
          <button
            disabled={!!connecting || step === "working"}
            onClick={() => signIn()}
          ...
          >
            ...
            <span className="flex-1">{step === "working" ? "Waiting for passkey…" : "Sign in with passkey"}</span>
            <span className="text-xs text-muted-foreground">Face ID / fingerprint</span>
          </button>
          <button
            disabled={!!connecting || step === "working"}
            onClick={() => signUp()}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-muted disabled:opacity-50 text-sm font-medium text-left transition-colors"
          >
            <span className="h-6 w-6 rounded-full border border-input flex items-center justify-center text-muted-foreground text-xs flex-shrink-0">+</span>
            <span className="flex-1">Create account</span>
          </button>
          {turnkeyError && <p className="px-3 py-1.5 text-xs text-red-500">{turnkeyError}</p>}
